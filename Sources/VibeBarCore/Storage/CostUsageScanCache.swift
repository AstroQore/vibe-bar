...
/// session log, keyed by file path. Each entry carries a fingerprint
...
public struct CostUsageScanCache: Codable, Sendable {
    ...
    public init(entries: [String: FileEntry] = [:]) {
    ...
    public func reusable(for path: String, mtime: Date, size: Int64) -> [ParsedEvent]? {
    ...
    public mutating func store(_ events: [ParsedEvent], for path: String, mtime: Date, size: Int64) {
        entries[path] = FileEntry(mtime: mtime, size: size, events: events)
    }
    ...
    public static func load(homeDirectory: String, tool: ToolType) -> CostUsageScanCache {
        let url = fileURL(homeDirectory: homeDirectory, tool: tool)
        if let attrs = try? FileManager.default.attributesOfItem(atPath: url.path),
           let size = (attrs[.size] as? NSNumber)?.intValue,
           size > maxFileBytes {
            return CostUsageScanCache()
        }
