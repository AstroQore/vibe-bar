...
public struct ClaudeSessionAdapter: SessionProviderAdapter {
    ...
    public func extractMetadata(fileURL: URL) throws -> SessionSummary {
        try Self.summary(fileURL: fileURL, provider: .claude, harness: .claudeCode)
    }

    /// Shared by Claude Code and Claude Cowork: the two write the same
    /// JSONL, and only the root they live under — and therefore the harness
    /// they bill against — differs.
    static func summary(
        fileURL: URL,
        ...
        return SessionSummary(
            ...
            sessionID: sessionID,
            harness: harness,
            model: model(in: head) ?? model(in: tail),
    ...
    /// `message.model` off the first assistant turn in the window. Claude
    /// Code writes `<synthetic>` for its own locally generated turns (the
    /// "no conversation found" notices), which is not a model anyone ran.
    static func model(in lines: [[String: Any]]) -> String? {
        for line in lines {
            guard let message = line["message"] as? [String: Any],
                  let model = SessionParsing.string(message["model"]),
    ...
    private static func firstRealUserText(_ lines: [[String: Any]]) -> String? {
        for line in lines {
    ...
    private static func lastMessageText(_ tail: [[String: Any]]) -> String? {
    ...
    public func parseTranscript(fileURL: URL, range: Range<Int>?) throws -> TranscriptDocument {
        try Self.transcript(fileURL: fileURL, range: range)
    }

    static func transcript(fileURL: URL, range: Range<Int>?) throws -> TranscriptDocument {
