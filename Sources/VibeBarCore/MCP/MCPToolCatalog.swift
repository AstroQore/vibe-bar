...
public enum MCPToolCatalog {
    ...
    public static let quotaGet = MCPTool(
        ...
        description: """
            ...
            SubProvider (ChatGPT Agentic, Claude, Gemini Web, AntiGravity, Grok, Cursor, Muse Code, Muse), L3 \
            buckets ('5 Hours', 'Weekly', per-model groups). Answers 'how much Codex / Claude / Gemini / \
            Grok / Cursor / Muse Code / Muse do I have left'. Percentages come back both used and remaining. 'lastUpdated' is when the numbers \
    ...
    public static let statusGet = MCPTool(
        ...
        description: """
            ...
            Cursor, Meta AI reads Meta's Model API status for Muse Code (Muse, the personal agent, has \
            no public status page). Useful when a provider looks broken and you want to know whether it is them or the \
            local setup.
            """,
        inputSchema: object(properties: ["tools": toolFilter])
    )
