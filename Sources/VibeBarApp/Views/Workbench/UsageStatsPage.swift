...
struct UsageStatsPage: View {
    ...
    private var noHarnessCard: some View {
        CardShell(density: density, alignment: .center) {
            ...
            Text(L10n.Usage.noHarnessSelectedTitle)
                .font(.system(size: density.titleFontSize, weight: .semibold))
                .multilineTextAlignment(.center)
            Text(L10n.Usage.noHarnessSelectedDetail)
    ...
    private var unavailableCard: some View {
        CardShell(density: density, alignment: .center) {
            Image(systemName: "externaldrive.badge.exclamationmark")
                .font(.system(size: 26, weight: .light))
                .foregroundStyle(.secondary)
            Text(L10n.Usage.ledgerUnavailableTitle)
                .font(.system(size: density.titleFontSize, weight: .semibold))
            Text(L10n.Usage.ledgerUnavailableDetail)
                .font(.system(size: density.subtitleFontSize))
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 420)
