...
Run after changing ToolType.hierarchy, the group-label table in
MiniWindowGroupLabelCatalog, the grouping rules in MenuBarQuotaGrouping, or the
...
def group_key_rules() -> dict:
    ...
    # The rules themselves live in Core so the menu bar's group merge can be
    # tested without AppKit; the App-side catalog only forwards to them.
    source = read("Sources/VibeBarCore/Models/MenuBarQuotaGrouping.swift")
    body = slice_between(source, "static func quotaGroupKey(", "static func namingGroupKey")
    ...
    defaults = dict(
        re.findall(r'case \.(\w+): return "([^"]+)"',
                   slice_between(source, "static func namingGroupKey", "static func allModelsGroupKey"))
    )
    ...
    branch = slice_between(
        read("Sources/VibeBarCore/Models/MenuBarQuotaGrouping.swift"),
...
def main() -> None:
    ...
    document = {
        ...
        "note": (
            ...
            "MenuBarQuotaGrouping.swift, MiniWindowGroupLabelCatalog.swift and "
            "QuotaFieldRegistry.swift. "
