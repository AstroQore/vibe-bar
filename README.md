...
clears. **Device groups** run two or more panels as a single display: they
share one row in the display list, one set of settings and one page list, and
each page chooses whether the screens draw one template **combined** across
them or a **separate** template each. Quota selections automatically continue
onto additional pages instead of stopping at five items. See [device group
setup and timing limits](docs/eink-screen-groups.md).
...
- [LiteLLM](https://github.com/BerriAI/litellm),
  ...
  carries the small Vibe Bar-specific supplement layer, plus the few
  corrections that outrank a wrong public price, and
