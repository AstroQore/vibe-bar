...
一键恢复的本地 Agent 会话索引，以及一个把同一份 Skill 库对账到八个 Agent CLI 的
...
可以只显示**单张**，也可以用**轮播**在多张之间切换。轮播有两种走法：**Device
loop** 让面板自己在若干个槽位之间轮转，Vibe Bar 负责把每个槽位填好；**App timer**
只占一个槽位，由 App 按你设定的节奏换里面的内容。设置页会按 1:1 预览每一张，真正
推送之前你就能看到效果；而且只有内容确实变了才会推送，数字没变的面板不会被叫醒
重画一遍。

告警会追加为一张轮播卡片，解除后移除。**设备组**可以把两块及以上屏幕当作一台
显示器：在同一个列表里共用一套设置和一份页面列表，每一页可以选择整组**合并显示**
同一个模板，或每屏**分屏显示**各自的模板。选择的内容超过单页容量会自动分页，
不限制只能选 5 项。Mac 会统一调度，但逐台发送请求，硬件完成刷新的时刻可能不同；
组内每台设备请只保留一个 Canvas API 轮播任务。
详见[设备组配置与刷新限制](docs/eink-screen-groups.md)。

**有两件事必须先在 Dot. App 里做好**，这两件都不能从 Vibe Bar 这边创建：

1. **一个 API Key**，在 Dot. App 的开发者设置里创建。
2. **在设备的 LOOP 任务里至少加一个「Canvas API」条目。** 接口只能往已经存在的
   位置里写，建不出新的；**固定任务不行**，必须是 LOOP 任务。需要几个取决于播放
   方式：单张和 **App timer** 轮播都只要一个——由 App 决定那一个槽位里放什么；
   **Device loop** 轮播则要**每张幻灯片一个**，因为轮转的是面板自己在真实槽位之间
   切换。多出来的槽位不会被浪费，但也不是白放：Vibe Bar 会把每个多余槽位占住并标
   成未使用，免得它一直显示昨天的数字。Vibe Bar 会扫描 loop 并告诉你找到了几个，
   补齐之后重新扫描即可。
...
**你的配额与用量数字会经过 Dot. 的服务器。** 面板没有本地接口：渲染好的幻灯片
——它画的配额百分比、重置时间、Token 与成本数字，以 JSON 形式——会带着你的 Dot.
API Key 发到 `dot.mindreset.tech`，由那边画到你的设备上。在你填入 key 并打开这个
功能之前什么都不会发生；Vibe Bar 也没有别的功能会把你的配额或用量发到任何地方
（App 其余的外发路径见下面的隐私清单）。如果你不接受这个取舍，把它关着就好——其余
的一切照常工作。
...
同一个 `MAJOR.MINOR`，每次 feature release 由两个仓库同时发布；patch 版本各自独立，
任何一边都可以自己修 bug 而不必等对方。这不改变本仓库的地位：这里是完整实现，
也是"对齐"这件事的参照标准。
...
| 观测历史与预测历史 | ● | ◐ | 跨平台版已记录观测并绘制重置历史条带；带导航条的配额历史图尚未移植 |
| 服务状态来源 | ● 5 | ● 4 | |
| **菜单栏 / 托盘** |
...
| **主窗口** |
| Provider 详情页 | ● 4 | ◐ | 跨平台版已按公司分页，含该公司的配额与状态；原生版的页面还带着该 Provider 的成本卡与历史图 |
| 可编排的模块瀑布流 | ● 11 | ○ | |
| 带预设的布局编辑器 | ● | ○ | |
| **迷你窗口** |
| 布局 | ● 7 | ◐ 1 | 环形、紧凑、账本、条带、磁贴、聚焦、轨道。跨平台版已有环形 |
| 多个独立窗口 | ● | ○ | |
| 半透明表面 | ● Liquid Glass | ○ | 计划用各平台自身的模糊效果，刻意不做复刻。目前窗口是不透明的无边框窗 |
| **工作台** |
| 用量图表、环图、明细表 | ● | ○ | 跨平台版已有重置历史条带，但没有用量图表 |
| 会话删除 | ● | ○ | |
| 重置：风险视图 | ● | ◐ | 跨平台版列出各次重置及其预测；日历视图与风险分组尚未移植 |
| 技能：安装、导入、发现、备份 | ● | ◐ | 跨平台版目前是只读清单 |
| **成本与用量** |
| 本地用量扫描 | ● 10 个 harness | ◐ 3 | Codex、Claude Code、Gemini CLI。只计有本地扫描器的 harness：Cursor 的用量来自 dashboard 事件，Grok Bot 根本没有用量来源，两者在任何一边都不算本地扫描 |
...
| MCP 工具 | ● 12 | ◐ 5 | 只读子集 |
| 远端 Probe 同步 | ● | ○ | |
| 墨水屏（Dot. Quote/0） | ● | ○ | 按计划把配额与用量推送到配对的面板 |
| 开机启动 | ● | ○ | |
...
- 墨水屏是唯一会把你的配额或用量数字发到别处的功能，而且只在你填入 key 并打开它
  之后才会。面板没有本地接口，所以每一张渲染好的幻灯片都会带着你的 Dot. API Key
  发到 `dot.mindreset.tech`，由那边画到设备上。Key 存在 Keychain Vault 里；
  `eink_state.json` 只保存上次画了什么的摘要和面板上次报告的状态。
- Vibe Bar 其余的外发都很窄，而且每一项都由你自己发起：Skills 搜索会把你的查询
  发给 `skills.sh`；纳管一台远端机器会提交它的名字和公钥；远端同步会向 Relay 提交
  确认游标，而 Relay 读不到它转发的内容。配额刷新则是把你自己配置的凭据发给对应
  的服务商，除此之外没有别的。
