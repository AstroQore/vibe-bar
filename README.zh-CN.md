...
**这是唯一一个会把你的数字发出去的功能。** 面板没有本地接口：每一张幻灯片都是带
着你的 key 发到 Dot. 云端（`dot.mindreset.tech`），由它渲染到你的设备上。也就是
说，幻灯片上的配额百分比、重置时间、Token 与成本数字会离开这台 Mac、到达 Dot. 的
服务器；除此之外没有别的东西外发，而且在你填入 key 并打开这个功能之前，什么都不
会发生。如果你不接受这个取舍，把它关着就好——Vibe Bar 其余的一切照常工作。
...
- 墨水屏是唯一会把数据发出这台 Mac 的功能，而且只在你填入 key 并打开它之后才会。
  面板没有本地接口，所以每一张幻灯片——上面的配额、重置时间与成本数字——都会发到
  Dot. 云端，由它渲染到你的设备上。Dot. API Key 存在 Keychain Vault 里；
...
`~/.vibebar/`、Keychain Vault 和上面明确列出的 Skills allowlist；唯一由它主动外发的
数据，是你要求它画的那张墨水屏幻灯片。完整取舍见
