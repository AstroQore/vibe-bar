...
告警会追加为一张轮播卡片，解除后移除。**屏幕编组**可以把多块屏幕按相对位置组成
大画布，使用同一套幻灯片操作显示独立页面、跨屏页面或混排。设备和编组分别管理，
选择的内容超过单页容量会自动分页，不限制只能选 5 项。Mac 会统一调度，但逐台发送
请求，硬件完成刷新的时刻可能不同；组内每台设备请只保留一个 Canvas API 轮播任务。
详见[编组配置与刷新限制](docs/eink-screen-groups.md)。
